...
export function OutputPanel({
  ...
  const { t } = useLanguage();
  const currentTitle = title === "Export Queue" ? t.outputTitle : title;
    ...
    <AnimatePresence>
      ...
      >
        ...
        {percentageSaved !== null && percentageSaved > 0 && (
          <span className="text-[10px] font-bold text-accent-secondary bg-accent-bg px-2 py-0.5 rounded-md">
            {t.outputReducedBy} -{percentageSaved}%
          </span>
        )}
      </div>
      ...
      <div className="space-y-4">
        ...
        {originalSize !== undefined && (
          ...
          >
            <div>
              <span className="block text-text-muted text-[9px] uppercase font-bold tracking-wider">{t.outputBefore}</span>
            ...
            {compressedSize !== undefined && (
              <div className="border-l border-brand-border/80 pl-3">
                <span className="block text-text-muted text-[9px] uppercase font-bold tracking-wider">{t.outputAfter}</span>
            ...
            {percentageSaved !== null && (
              <div className="hidden sm:block border-l border-brand-border/80 pl-3">
                <span className="block text-text-muted text-[9px] uppercase font-bold tracking-wider">{t.outputSavings}</span>
                <span className="font-mono text-accent-secondary mt-1.5 block font-bold">-{percentageSaved}% {t.outputShrink}</span>
              </div>
            )}
          </motion.div>
        )}
        ...
        {onDownloadAll && (
          ...
          >
            ...
            {isProcessing ? t.outputAssembling : (downloadLabel === "Download output package" ? t.outputTitle : downloadLabel)}
          </motion.button>
        )}
      </div>
      </motion.div>
